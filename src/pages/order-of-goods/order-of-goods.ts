...
import { UnOrderInformationPage } from '../un-order-information/un-order-information';
// import { DeliveryPage } from '../delivery/delivery';
// import { ShippingPage } from '../shipping/shipping';
// import { DeliveryDetailPage } from '../delivery-detail/delivery-detail';
import { WaitPaymentPage } from '../../pages/wait-payment/wait-payment';
...
export class OrderOfGoodsPage {
  ...
  allOrder(item) {
   // event.stopPropagation();
   if(item.storeShow==false){
    this.navCtrl.push('PaymentPage', { 'custgoodsorderid': item.custgoodsorderid, 'totalprice': item.totalgoodsprice ,'type':"orderservice"});
   }else{
    if(item.status==1){
  ...
  chaxun(infiniteScroll) {
    ...
    this.http.httpMethodContext(HttpUrl['querygoodsorder'], data, (res, context) => {
      ...
      if (context.flipper_page == 0) {
        context.goodsorder = [];
        for(let item of res.result.rows){
          if(item.ordertype==30){
            console.log('购买服务=='+item.ordertype)
            item.storeShow=false
            if (item.status == 1) {
              item.btnText = '待付款';
              item.status_class = 'item_lineHight bg-red';
              item.this_order = '去付款';
              item.hasbutton =true;
            } else if (item.status == 2 ||item.status ==3 ||item.status == 8 || item.status == 9 ||item.status == 10) {
              item.btnText = '待收货';
              item.hasbutton =true;
              item.status_class = 'item_lineHight bg-blue';
              item.this_order = '确认收货';
            } else if (item.status == 4 ||item.status == 7) {
              item.btnText = '已完成';
              item.hasbutton =true;
              item.status_class = 'item_lineHight bg-gray';
              item.this_order = '再来一单';
            } else if (item.status == 11) {
              item.btnText = '退款中';
              item.hasbutton =true;
              item.status_class = 'item_lineHight bg-green';
              item.this_order = '发货退货';
            }else if (item.status == 6||item.status == 12) {
              item.btnText = '退款中';
              item.hasbutton =false;
