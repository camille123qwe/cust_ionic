...
export class GoodsxiangqingPage {
  ...
  storeIMg = ''; storeName = ''; disTance = ''; storeAddr = ''; thisStore;
  hotGoodRows = [];
  props;
  Opacity;
  tel_str = '';
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public zone: NgZone, private http: HttpGet, public modalCtrl: ModalController, ) {
    ...
    console.log('xxx000=='+JSON.stringify(this.navParams.data))
  ...
  scrollHandler(event) {
    this.zone.run(() => {
      ...
      if (scrollHeight < bannersHeight - hasshow) {

        this.divShow = true;
      } else {
        this.divShow = false;
        console.log('进入时高度' + this.content.getContentDimensions().scrollTop)
        console.log(bannersHeight)
      }
  ...
  // 查询商品详情
  goodsDetail() {
    let url = HttpUrl.aloneGoods + this.goodsdata.goodsInfo.goodsid.toString(36);
  ...
  ionViewDidLoad() {
    ...
    // let money=this.serveData-(this.sellPrice*this.goodcunt)
  ...
  toFollow() {
    ...
    if (custuserid == null || custuserid === 'undefined') {
      ...
      let url = HttpUrl.followShop + this.goodsdata.goodsInfo.storeid.toString(36);  //#	int	门店36ID
      this.http.httpMethodContext(url, {}, (res, context) => {
        showToast('关注成功');
  ...
  responseData(item) {
    if (typeof (item) == 'undefined' || item == '' || 　item == '无') {
      return false;
    } else {
      return true;
    }
  ...
  addgoods() {
    ...
    // myModal.present();
    // {"afterDelte":{"bean":"{\"cartgoodsitemsid\":\"\",\"custuserid\":\"200000001\",\"goodsid\":100000753,\"count\":1,\"storeid\":25000460}
    // ","cols":[],"props":"[]"}}
    let custuserid = localStorage.getItem('custuserid');
    if (custuserid == null || custuserid === 'undefined') {
      console.log('未登录');
      ...
      let modal = this.modalCtrl.create('DeliveryPopupPage', this.goodsdata);
  ...
  addToCart() {
    ...
    let data = {
      'bean': {
        ...
        goodsid: this.goodsdata.goodsInfo.goodsid,
        count: 1,
        storeid: this.goodsdata.goodsInfo.storeid,
        // status:'',//		short
        // props:''
      }, 'cols': '[""]'
      , 'props': [],
  ...
  //查询门店
  oneShop() {
    let url = HttpUrl.oneShopInfo + this.goodsdata.goodsInfo.storeid.toString(36)
    this.http.httpMethodContext(url, {}, (res, context) => {
      console.log('wwwoodk=='+res.seen)
      if (typeof (context.navParams.get('type')) !== 'undefined') {
        
        if (res.seen==4) {
          console.log(this.navParams.get('type'))
          if (context.navParams.get('type') == 'alonegoods') {
            context.ifStore = false;
            context.dingjin = false;
          } else {
            context.ifStore = false;
            context.dingjin = true;
          }
  ...
  hotGoods() {
    // shopGoods
    console.log('store36id===' + this.goodsdata.goodsInfo.storeid.toString(36))
    let data = {
      'bean': {
        storeid: this.goodsdata.goodsInfo.storeid,
        status: [10],
  ...
  // goodsInfo{}
  custmerchdelivery() {
    this.http.httpMethodContext(HttpUrl.custmerchdelivery + this.goodsdata.goodsInfo.merchid.toString(36), {}, (res, context) => {

      if (res.rows.length == 0 || typeof (res.rows) == 'undefined' || res.rows == '' || typeof (res.rows[0].serviceDistance) == 'undefined' || res.rows[0].serviceDistance == '') {
        return;
      } else {
  ...
  share() {
    console.log(this.goodsdata.goodsInfo.goodsname);
    console.log(this.goodsdata.goodsInfo.storeid);
    console.log(this.goodsdata.goodsInfo.goodsid)
    console.log(this.goodsface_900 + this.goodsdata.goodsInfo.goodsface);
    ...
    (<any>window).Cordova.exec((res) => {
      ...
      [{
        ...
        "shareTitle": "分享内容",
        "title": this.goodsdata.goodsInfo.goodsname,
        "text": "价格实惠，还获得了神秘礼品，赚到了，分享给你们",
        "url": "http://merch.diancall.com/modules/share/sharegoods/goods.html" + "?StoreId=" + this.goodsdata.goodsInfo.storeid.toString(36) + "?GoodsId=" + this.goodsdata.goodsInfo.goodsid.toString(36) + "?TaskId=" + this.Taskid,
        "imagePath": this.goodsface_900 + this.goodsdata.goodsInfo.goodsface,
      }]);
  }
}
