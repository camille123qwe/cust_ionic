...
export class MyShopPage {
  ...
  ionViewDidEnter() {
    console.log('ionViewDidEnter MyShopPage');
    this.getData();
  }
  search(event) {
  ...
  getRecently(infiniteScroll) {
    ...
    this.http.httpMethodContext(HttpUrl.shopNearby, dataParams, (res, context) => {
      ...
      for (let i = 0; i < res.rows.length; i++) {
          ...
          if (typeof res.rows[i].store == 'undefined') {
            let store = { distance: "" };
            // console.log('iiiiiiiiii=='+res.rows.length)
            res.rows[i].store = store;
            res.rows[i].itemshow = true;
          } else {
            if (res.rows[i].store.distance === 0) {
  ...
  unFollow(item) {
    ...
    this.http.httpMethodContext(url, {}, (res, context) => {
      if (res.success) {
        // context.globalProvider.presentToast('取消关注成功');
        // console.log(this.familar_shops.rows.indexOf(item));
        console.log('已取消关注');
        showToast('已取消关注');
        switch (this.type) {
          case 'left':
