...
export class DeliveryPopupPage {
  ...
  init() {
    this.goodsInfos = this.navParams.data;
    if (this.goodsInfos.mincount) {
      this.count_number = this.goodsInfos.mincount;
    }
    console.log(JSON.stringify(this.goodsInfos.goodsInfo));
    // if (typeof (this.goodsInfos.goodsInfo) == 'undefined' || 　this.goodsInfos.goodsInfo.propValues.length == 0) {
    //   return;
    //   // this.specTitles = [];
    // } else {


    //   for (let item in this.goodsInfos.goodsInfo.propValues) {
    //     this.specTitles.push(item);
  ...
  selectSpec(content) {
    ...
    // console.log('PropIds==' + JSON.stringify(this.PropIds));
  ...
  confirm() {
    ...
    let params = {
      bean: {
        ...
        goodsid: this.goodsInfos.goodsInfo.goodsid,//		int	商品id
        count: this.count_number,//		int	件数
        storeid: this.goodsInfos.goodsInfo.storeid,//		int	商品关联门店id
      },
      cols: [],
