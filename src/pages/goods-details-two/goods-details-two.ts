...
@IonicPage()
@Component({
  selector: 'page-goods-details-two',
  templateUrl: 'goods-details-two.html',
})
export class GoodsDetailsTwoPage {
  ...
  detilsShow() {
    let bannersHeight = $('#goodbanner').height()
  ...
  toFollow() {
    ...
    if (custuserid == null || custuserid === 'undefined') {
      ...
      let url = HttpUrl.followShop + this.goodsdata.goods36id;  //#	int	门店36ID
  ...
  goodsDetail() {
    ...
    this.http.httpMethodContext(url, {}, (res, context) => {
      ...
      if (typeof (res.goodsimgs) == 'undefined' || res.goodsimgs == '') {
        ...
        if (context.goodImg.length >= 3) {
          console.log('goodImg.length' + context.goodImg.length)
          context.divShow = true;
        }
      }
  ...
  goNextPage(item) {
    console.log(JSON.stringify(item))
    this.navCtrl.push('GoodsDetailsThreePage', item)
  }
